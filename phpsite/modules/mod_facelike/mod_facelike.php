...
/*********************************************************************
* Author: Cico Zeljko
* @package www.artcreative.me Face Like Button Module
* @copyright Copyright (C) www.artcreative.me. All rights reserved.
* @license http://www.gnu.org, see LICENSE.php
* Version 1.2.1.0
*********************************************************************/
defined('_JEXEC') or die('Restricted access');
$likebox = facelike($params);
echo $likebox;
function facelike($params){
  	  global $mainframe;

      $url = JURI::current();

       $show_COPY = trim($params->get('artcreative'));
       if ($show_COPY =="0"){$notice='<br /><div style="font-size:7px;"><a href="http://www.artcreative.me">ArtCreativeFace Like Button Module</a> </div>';}
       if ($show_COPY =="1"){$notice='<br /><div style="display:none;"><a href="http://www.artcreative.me">ArtCreativeFace Like Button Module</a> </div>';}
       $style = trim( $params->get( 'like_tip' ) );
       if ($style =="0"){$style1='standard';}
       if ($style =="1"){$style1='button_count';}
       if ($style =="2"){$style1='box_count';}
       $slova = trim( $params->get( 'like_font' ) );
       if ($slova =="0"){$slova1='arial';}
       if ($slova =="1"){$slova1='lucida+grande';}
       if ($slova =="2"){$slova1='segoe+ui';}
       if ($slova =="3"){$slova1='tahoma';}
       if ($slova =="4"){$slova1='trebuchet+ms';}
       if ($slova =="5"){$slova1='verdana';}
       $color = trim( $params->get( 'like_color' ) );
       if ($color =="0"){$color1='light';}
       if ($color =="1"){$color1='dark';}
       $face = trim( $params->get( 'like_faces' ) );
       if ($face =="0"){$face1='true';}
       if ($face =="1"){$face1='false';}
       $verb = trim( $params->get( 'like_verb' ) );
       if ($verb =="0"){$verb1='like';}
       if ($verb =="1"){$verb1='recommend';}
       $auto_url = trim( $params->get( 'auto_url' ) );
       if ($auto_url =="0"){$autourl1=urlencode($url);}
       if ($auto_url =="1"){$autourl1=trim( $params->get( 'adresa' ) );}
...
$likebox='<div class="'.trim($params->get('moduleclass_sfx')).'"><iframe src="http://www.facebook.com/plugins/like.php?href='.$autourl1.'&amp;layout='.$style1.'&amp;show_faces='.$face1.'&amp;width='.trim( $params->get( 'like_width' ) ).'&amp;action='.$verb1.'&amp;font='.$slova1.'&amp;colorscheme='.$color1.'&amp;height='.trim( $params->get('like_height')).'&amp;locale='.$lang_locale1.'" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:'.trim( $params->get( 'like_width' ) ).'px; height:'.trim($params->get('like_height')).'px;" allowTransparency="true"></iframe>'.$notice.'</div>';
return $likebox;
}
?>
